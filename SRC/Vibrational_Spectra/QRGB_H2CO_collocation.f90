...
!==============================================================================!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! TODO: finish documentation for subroutines etc
! fix transformation order (currently it is backward)
...
!###  transformations begins here (Taken from Tucker)
  ...
  r = (/r3,r1,r2,acos(costheta1),acos(costheta2),acos(cosphi)/)
  ...
  bx(:)=0d0
  x(5)=r(1)                     ! y(O)
  x(7)=-sin(r(4))*r(2)          ! x(H1)
  x(8)=cos(r(4))*r(2)           ! y(H1)
  x(11)=cos(r(5))*r(3)          ! y(H2)
  ksi=sin(r(5))*r(3)            ! distance from H2 to the y axis
  x(10)=ksi*cos(pi-r(6))        ! x(H2)
  x(12)=ksi*sin(pi-r(6))        ! z(H2)
!###  transformation ends here
  !x=(0 , 0 , 0 , 0 , x5, 0 , x7, x8, 0, x10, x11, x12)
  !x(1:3) = x(1:3)*sqrt(Cmass)    ! it's already zero
  x(5) = x(5)*sqrt(Omass)
  x(7:12) = x(7:12)*sqrt(Hmass)   ! mass-scale back
  !x1=(/x(5),x(7),x(8),x(10),x(11),x(12)/)
  endif
...
!============================================================================!
if(flag) call XYZms_to_Internal(x,r1,.true.)  !transform to internal coordinates
...
implicit none
logical flag
double precision :: Vr, V
integer :: ix
double precision :: rr(6), x(12)
double precision :: C(3), O(3), H1(3), H2(3), r1vec(3), r2vec(3), r3vec(3)
double precision :: r1r3normvec(3), r2r3normvec(3), r1, r2, r3, costheta1
double precision :: costheta2,cosphi,r1r3norm,r2r3norm,ksi
!============================================================================!
if(flag) then !transform x(1:d) --> rr(1:d1)
!###  Tucker's transformation starts here
  C(:) = x(1:3)/sqrt(Cmass)          ! unscale the mass-scaled coordinates
  O(:) = x(4:6)/sqrt(Omass)
  H1(:) = x(7:9)/sqrt(Hmass)
  H2(:) = x(10:12)/sqrt(Hmass)
  ...
  rr = (/r3,r1,r2,acos(costheta1),acos(costheta2),acos(cosphi)/)

  if(dot_product(r1r3normvec,r2vec)>0d0) rr(6)=2*pi-rr(6)
!### End Tucker's transformation
endif
call h2copot_carter97(rr,Vr)    ! H2CO potential

if(Vr<0d0) then
  write(*,*) 'Killing Program! Vr < 0d0. Check Function V'
  write(*,*) 'V=',Vr
  write(*,*) 'r=',r
  write(*,*) 'x=',x
  STOP
